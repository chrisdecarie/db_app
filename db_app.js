async function fetchFavourites(url){
  ...
  function displayData(data){
    ...
    data.forEach((user)=>{
      ...
      a.innerHTML = " edit movie, food, drink";
      li.innerHTML = `${user.name} likes to watch ${user.movie}, eat ${user.food}, and drink ${user.drink}.`;
      li.appendChild(a);
      a.addEventListener('click', (event)=> {
        //wow lots of parameters
        updateForm(event, user.demoID, user.name, user.movie, user.food, user.drink);
      });
  ...
  function updateForm(event, demoID, user, movie, food, drink){
    console.log(demoID, user, movie, food, drink);
