...
$results = [];
  ...
  try{
    if(!isset($_REQUEST["full_name"]) || !isset($_REQUEST["email"]) || !isset($_REQUEST["movie"]) || !isset($_REQUEST["food"]) || !isset($_REQUEST["drink"])   ){
      throw new Exception('Required data is missing i.e. name, email, movie, food, drink');
    }else{
      $query = "INSERT INTO demo (name, email, movie, food, drink) VALUES (?, ?, ?, ?, ?)";

      if($stmt = mysqli_prepare($link, $query)){
        mysqli_stmt_bind_param($stmt, 'sss', $_REQUEST["full_name"], $_REQUEST["email"], $_REQUEST["movie"], $_REQUEST["food"],$_REQUEST["drink"]);
